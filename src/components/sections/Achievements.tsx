    ...
    <section id="achievements" className="min-h-screen flex items-center justify-center py-12 px-4">
      <div className="max-w-6xl w-full">
        <div className="relative">
          {/* Background Accents */}
          <div className="absolute -top-24 -right-24 w-72 h-72 bg-purple-500/3 rounded-full blur-3xl animate-float" 
               style={{ animationDuration: '8s' }} />
          <div className="absolute -bottom-24 -left-24 w-72 h-72 bg-yellow-500/3 rounded-full blur-3xl animate-float" 
            ...
            <div className="space-y-8">
              ...
              <div className="grid lg:grid-cols-2 gap-8 items-start">
                ...
                >
                  ...
                  <button
                    onClick={prevImage}
                    className="absolute left-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-slate-800/50 border border-slate-700/50 hover:bg-slate-700/50 transition-colors duration-300"
                    aria-label="Previous image"
                  >
                    <ChevronLeft className="w-5 h-5 text-slate-400 hover:text-yellow-400" />
                  </button>
                  <button
                    onClick={nextImage}
                    className="absolute right-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-slate-800/50 border border-slate-700/50 hover:bg-slate-700/50 transition-colors duration-300"
                    aria-label="Next image"
                  >
                    <ChevronRight className="w-5 h-5 text-slate-400 hover:text-yellow-400" />
                  </button>
...
const AchievementItem: React.FC<AchievementItemProps> = ({ achievement, index }) => {
  ...
  return (
    <div 
      ref={animation.ref}
      className={`bg-slate-800/30 backdrop-blur-sm rounded-lg p-4 border border-slate-700/50 hover:border-sky-500/50 transition-all duration-300 ${animation.animationClasses} group`}
    >
